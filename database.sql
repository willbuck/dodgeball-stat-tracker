...
SELECT
    TRUE,
    floor(random() * 33) + 1,  -- Random game_id between 1 and 33
    floor(random() * 36) + 1,  -- Random player_id between 1 and 36
    CASE
        WHEN s <= 9 THEN s  -- user_id from 1 to 9 for the first 9 rows
        ELSE floor(random() * 9) + 1  -- Random user_id between 1 and 9 for the remaining rows
    END,
    0,  -- Set kills to 0
    0,  -- Set outs to 0
    0  -- Set catches to 0
FROM generate_series(1, 50) s;  -- Generate series from 1 to 50
-- User table
CREATE TABLE "user" (
  id SERIAL PRIMARY KEY,
  username VARCHAR(90) NOT NULL,
